...
function EditQuestion() {
    ...
    useEffect(() => {
        setQuestionData(location?.state?.editData[0])
    }, [location])
    ...
    const handleSubmit = async () => {
        ...
        if (isValid) {
            try {
                await axios.put(`http://localhost:3001/${location?.state?.endpoint}/${location?.state?.editData[0].id}`, questionData);
    ...
    return (
        <div className='d-flex justify-content-center mt-3 mb-3 '>
            <div className='container card mb-3 w-75 px-5'>
                <h3 className='text-center mt-3'>Edit Question</h3>
                <div className='p-3'>
                    <div className='mb-2'>
                    ...
                    < div className='' >
                        {
                            questionData?.options?.map((val, index) => (
                                <div key={`${val}-${index}`} className='mt-1'>
